#pragma once

#include "Math\Quaternions\Quaternion.hpp"
#include "Math\Quaternions\DualQuaternion.hpp"

// using namespace glMath;

/// @brief shorthand for writing quat<float>
using quatf = glMath::quat<float>;
/// @brief shorthand for writing quat<double>
using quatd = glMath::quat<double>;

/// @brief shorthand for writing dualQuat<float>
using dualQuatf = glMath::dualQuat<float>;
/// @brief shorthand for writing dualQuat<double>
using dualQuatd = glMath::dualQuat<double>;

/// @brief shorthand for writing dualQuat<float>
using dQuatf = glMath::dualQuat<float>;
/// @brief shorthand for writing dualQuat<double>
using dQuatd = glMath::dualQuat<double>;

